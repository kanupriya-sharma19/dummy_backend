...
export async function getUsers(req: Request, res: Response) {
  ...
  res.json({ status: true, message: "Users Successfully fetched", data: users });
}
...
export const signupUser = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    const user = await prisma.user.create({ data: { name, email, password: hashedPassword } as Prisma.UserUncheckedCreateInput });

    const token = jwt.sign({ id: user.id }, process.env.JWT_SECRET as string, { expiresIn: '1h' });
    res.cookie('token', token, { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 3600000 });

    res.status(201).json({ status: true, message: 'User successfully signed up', data: { user } });
...
export async function updateProfile(req: Request, res: Response) {
  try {
    ...
    const updatedUser = await prisma.user.update({
      where: { id: req.user },
      data: { dob: dob ? new Date(dob) : undefined, gender, city, gamePreferences: gamePreferences || undefined, profilePhoto },
    });

    res.json({ status: true, message: 'Profile updated successfully', data: updatedUser });
  } catch (error: any) {
...
export async function resetPassword(req: Request, res: Response): Promise<any> {
  ...
  const isTokenExpired = user.resetTokenExpiration && user.resetTokenExpiration < new Date();
  if (isTokenExpired) {
